...
class renderer extends \core_user\output\myprofile\renderer {
    ...
    public function render_tree(tree $tree) {
        ...
        $output .= html_writer::start_tag('div', array('id' => 'generalInformation'));
        $output .= html_writer::empty_tag('img', array('class' => 'iconLabel', 'src' => $this->image_url('info', 'theme'))); 
        $output .= html_writer::start_tag('h3');
        $output .= "Superpoderes";
        $output .= html_writer::end_tag('h3');
        $output .= html_writer::start_tag('div', array('id' => 'containerChart'));
        $output .= html_writer::start_tag('canvas', array('id' => 'chart'));
